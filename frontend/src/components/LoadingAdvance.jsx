                            ...
                            <CardContent className="space-y-4">
                                {form.invoices.map((inv, idx) => (
                                    <div key={idx} className="grid grid-cols-1 md:grid-cols-2 gap-4 p-3 rounded-lg border border-slate-100">
                                        ...
                                        {form.product_name && <div className="space-y-1"><Label className="required">KT Freight</Label><Input type="number" step="0.01" value={inv.kt_freight} onChange={e => updateInvoice(idx, { kt_freight: e.target.value })} /></div>}
