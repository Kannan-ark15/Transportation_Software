...
CREATE INDEX IF NOT EXISTS idx_loan_repayment_trackings_loan_master_id ON loan_repayment_trackings(loan_master_id);
CREATE INDEX IF NOT EXISTS idx_loan_repayment_trackings_due_settled ON loan_repayment_trackings(due_settled);
...
CREATE TABLE IF NOT EXISTS cashbook_payments (
    ...
    reference_record_type VARCHAR(40) NOT NULL DEFAULT 'Settlement',
    reference_record_id INTEGER NOT NULL,
    reference_loading_advance_ids INTEGER[],
    reference_amount_snapshot DECIMAL(12, 2),
...
CREATE INDEX IF NOT EXISTS idx_cashbook_payments_reference_loading_advance_ids ON cashbook_payments USING GIN(reference_loading_advance_ids);
CREATE INDEX IF NOT EXISTS idx_cashbook_payments_created_by ON cashbook_payments(created_by);
CREATE UNIQUE INDEX IF NOT EXISTS idx_cashbook_payments_reference_unique ON cashbook_payments(reference_module, reference_record_type, reference_record_id);
