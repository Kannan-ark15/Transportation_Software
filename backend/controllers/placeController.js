...
class PlaceController {
    static async createPlace(req, res) {
        try {
            ...
            const exists = await PlaceModel.existsCombination(placeData.company_id, placeData.to_place, placeData.product_id);
            if (exists) {
                return res.status(400).json({ success: false, message: 'This Company, To Place and Product combination already exists' });
            }
    ...
    static async updatePlace(req, res) {
        try {
            ...
            const exists = await PlaceModel.existsCombination(placeData.company_id, placeData.to_place, placeData.product_id, id);
            if (exists) {
                return res.status(400).json({ success: false, message: 'This Company, To Place and Product combination already exists' });
            }
